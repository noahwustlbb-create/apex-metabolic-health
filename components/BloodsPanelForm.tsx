      ...
      <div className="p-5 rounded-sm mb-5" style={{ background: 'rgba(72,144,247,0.04)', border: '1px solid rgba(72,144,247,0.12)' }}>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-xs" style={{ color: '#4a5878' }}>{config.programLabel} Program</p>
          </div>
...
function Success({ firstName, config }: { firstName: string; config: PanelConfig }) {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }} className="max-w-lg">
      ...
      <div className="p-5 rounded-sm mb-6" style={{ background: '#ffffff', border: '1px solid rgba(72,144,247,0.1)' }}>
        <p className="text-[10px] font-bold tracking-[0.18em] uppercase mb-4" style={{ color: '#7a90a8' }}>WHAT HAPPENS NEXT</p>
          ...
          <div ref={topRef} className="container-tight relative z-10" style={{ maxWidth: 680 }}>
            ...
            {submitted ? <Success firstName={data.firstName} config={config} /> : (
              <>
                ...
                <p className="text-xs mt-5" style={{ color: '#7a90a8' }}>Your information is private and confidential. Doctor-issued referral only — no walk-ins required.</p>
              </>
            )}
          </div>
        </section>
