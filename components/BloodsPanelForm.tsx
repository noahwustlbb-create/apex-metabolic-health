  ...
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        ...
        style={{ background: 'rgba(200,169,110,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: '#F5F5F5', caretColor: '#C8A96E' }}
        onFocus={e => { e.target.style.borderColor = 'rgba(200,169,110,0.5)'; e.target.style.background = 'rgba(200,169,110,0.04)' }}
...
function Step1({ data, set }: { data: FormData; set: (k: keyof FormData, v: string) => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <h2 className="text-3xl font-bold tracking-tight mb-1" style={{ fontFamily: 'var(--font-space-grotesk)', color: '#F5F5F5' }}>Your details</h2>
      <p className="text-sm mb-8" style={{ color: '#888888' }}>We&apos;ll use these to issue your referral and send your results.</p>

      {/* Morning collection notice */}
      <div className="flex gap-3 p-4 rounded-sm mb-8" style={{ background: 'rgba(200,169,110,0.06)', border: '1px solid rgba(200,169,110,0.2)' }}>
        <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4 flex-shrink-0 mt-0.5" aria-hidden="true">
          ...
          <path d="M12 8v4m0 4h.01" stroke="#C8A96E" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
        <p className="text-xs leading-relaxed" style={{ color: '#7ab8b5' }}>
          <strong style={{ color: '#C8A96E' }}>Morning collection required.</strong> For accurate hormone and metabolic markers, attend your collection centre fasted (10–12 hrs) before 9am. Drink water only.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-semibold tracking-[0.12em] uppercase mb-1.5" style={{ color: '#888888' }}>State <span style={{ color: '#C8A96E' }}>*</span></label>
          <select value={data.state} onChange={e => set('state', e.target.value)}
            ...
            style={{ background: 'rgba(200,169,110,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: data.state ? '#F5F5F5' : '#4a5a6a' }}>
...
function Step2({ data, set, toggleCondition }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <h2 className="text-3xl font-bold tracking-tight mb-1" style={{ fontFamily: 'var(--font-space-grotesk)', color: '#F5F5F5' }}>Medical notes</h2>
      <p className="text-sm mb-8" style={{ color: '#888888' }}>Reviewed by your doctor before issuing your referral.</p>
      <div className="flex flex-col gap-6">
        <div>
          <label className="block text-xs font-semibold tracking-[0.12em] uppercase mb-2" style={{ color: '#888888' }}>Medical conditions (select all that apply)</label>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {CONDITIONS.map(c => (
              <button key={c} type="button" onClick={() => toggleCondition(c)}
                className="px-3 py-2 rounded-sm text-xs font-medium transition-all duration-150 text-left"
                style={{
                  background: data.conditions.includes(c) ? 'rgba(200,169,110,0.08)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${data.conditions.includes(c) ? 'rgba(200,169,110,0.35)' : 'rgba(255,255,255,0.07)'}`,
                  color: data.conditions.includes(c) ? '#C8A96E' : '#8899aa',
                }}>{c}</button>
            ))}
          </div>
        </div>
...
function Step3({ data, set, config }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <h2 className="text-3xl font-bold tracking-tight mb-1" style={{ fontFamily: 'var(--font-space-grotesk)', color: '#F5F5F5' }}>Confirm & submit</h2>
      <p className="text-sm mb-8" style={{ color: '#888888' }}>Review your panel and confirm consent.</p>

      {/* Panel summary */}
      <div className="p-5 rounded-sm mb-5" style={{ background: 'rgba(200,169,110,0.05)', border: '1px solid rgba(200,169,110,0.15)' }}>
        <div className="flex justify-between items-start mb-4">
          <div>
            <p className="text-xs font-bold tracking-[0.16em] uppercase mb-1" style={{ color: '#C8A96E' }}>{config.panelName}</p>
            <p className="text-xs" style={{ color: '#888888' }}>{config.programLabel} Program</p>
          </div>
          <span className="text-lg font-bold" style={{ fontFamily: 'var(--font-space-grotesk)', color: '#F5F5F5' }}>{config.panelPrice}</span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {config.biomarkers.map(b => (
            <span key={b} className="px-2 py-0.5 rounded-sm text-[10px] font-medium" style={{ background: 'rgba(200,169,110,0.08)', color: '#5ab8b5', border: '1px solid rgba(200,169,110,0.15)' }}>{b}</span>
          ))}
        </div>
      </div>

      {/* Personal summary */}
      <div className="p-5 rounded-sm mb-6" style={{ background: '#111111', border: '1px solid rgba(200,169,110,0.12)' }}>
        ...
        {[
          ...
          <label key={key} className="flex items-start gap-3 cursor-pointer">
            <div onClick={() => set(key, !(data[key] as boolean))}
              ...
              style={{
                background: data[key] ? 'rgba(200,169,110,0.12)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${data[key] ? 'rgba(200,169,110,0.45)' : 'rgba(255,255,255,0.1)'}`,
              }}>
              {data[key] && <svg viewBox="0 0 12 12" fill="none" className="w-3 h-3"><path d="M2 6l3 3 5-5" stroke="#C8A96E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
            </div>
...
function Success({ firstName, config }: { firstName: string; config: PanelConfig }) {
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }} className="max-w-lg">
      <div className="w-11 h-11 flex items-center justify-center rounded-sm mb-7" style={{ background: 'rgba(200,169,110,0.1)', border: '1px solid rgba(200,169,110,0.25)' }}>
        <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5"><path d="M5 12l5 5L19 7" stroke="#C8A96E" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </div>
      <p className="label mb-3">ORDER RECEIVED</p>
      <h2 className="text-3xl font-bold tracking-tight mb-4" style={{ fontFamily: 'var(--font-space-grotesk)', color: '#F5F5F5' }}>
      ...
      <div className="p-5 rounded-sm mb-6" style={{ background: '#111111', border: '1px solid rgba(200,169,110,0.12)' }}>
        <p className="text-[10px] font-bold tracking-[0.18em] uppercase mb-4" style={{ color: '#666666' }}>WHAT HAPPENS NEXT</p>
        <div className="flex flex-col gap-3">
          {[
            ...
            <div key={i} className="flex items-start gap-3">
              <div style={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, background: 'rgba(200,169,110,0.08)', border: '1px solid rgba(200,169,110,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 800, color: '#C8A96E', fontFamily: 'var(--font-space-grotesk)' }}>{i + 1}</div>
              <p className="text-sm leading-relaxed" style={{ color: '#888888' }}>{s}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <div ref={topRef} className="container-tight relative z-10" style={{ maxWidth: 680 }}>
            ...
            {!submitted && (
              <div className="mb-10">
                <div className="flex justify-between mb-2">
                  <span className="text-xs font-semibold tracking-[0.16em] uppercase" style={{ color: '#C8A96E' }}>
                    {config.panelName} — {config.panelPrice}
                  </span>
                  <span className="text-xs" style={{ color: '#666666' }}>{progress}%</span>
                </div>
                <div style={{ height: 2, background: 'rgba(255,255,255,0.06)', borderRadius: 2 }}>
                  <motion.div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg, #C8A96E, #C8A96E)' }}
                    initial={false} animate={{ width: `${progress}%` }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} />
                </div>
                <div className="flex gap-2 mt-3">
                  {STEPS.map((s, i) => <span key={s} className="text-[10px] tracking-wide" style={{ color: i + 1 <= step ? '#C8A96E' : '#2a3a4a' }}>{s}</span>)}
                </div>
              </div>
            )}
            ...
            {submitted ? <Success firstName={data.firstName} config={config} /> : (
              <>
                ...
                <div className="flex items-center gap-4 mt-8">
                  {step > 1 && (
                    <button type="button" onClick={() => { setStep(s => s - 1); scrollTop() }}
                      ...
                      style={{ background: 'rgba(200,169,110,0.04)', border: '1px solid rgba(200,169,110,0.12)', color: '#888888' }}>← Back</button>
                  )}
                  {step < 3 ? (
                    <button type="button" onClick={() => { setStep(s => s + 1); scrollTop() }}
                ...
                <p className="text-xs mt-5" style={{ color: '#666666' }}>Your information is private and confidential. Doctor-issued referral only — no walk-ins required.</p>
              </>
            )}
          </div>
        </section>
