    ...
    <>
      ...
      >
        <div className="mx-auto w-full max-w-[1440px] px-6 md:px-8 flex items-center justify-between h-[68px] md:h-[76px]">
              ...
              <AnimatePresence>
                {programsOpen && (
                  <motion.div
                    ...
                    style={{ width: 300, background: '#ffffff', border: '1px solid rgba(72,144,247,0.14)', borderRadius: 12, boxShadow: '0 16px 48px rgba(72,144,247,0.1), 0 2px 12px rgba(0,0,0,0.06)', padding: '8px' }}
                  >
          ...
          {/* Mobile hamburger */}
          <button className="md:hidden flex flex-col gap-[5px] p-2" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation menu" aria-expanded={menuOpen}>
                ...
                {[
                  ...
                  <motion.div key={link.href} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.26, delay: i * 0.055 + 0.04 }}>
                    <Link href={link.href} onClick={() => setMenuOpen(false)}
                      ...
                      style={{ color: 'rgba(10,14,26,0.75)', borderColor: 'rgba(72,144,247,0.1)', fontFamily: 'var(--font-space-grotesk)', letterSpacing: '-0.01em' }}
                    >
                      {link.label}
